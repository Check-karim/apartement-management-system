...
import { z } from "zod";
import { ArrowLeft, Send, Droplet, Building } from "lucide-react";
import toast from "react-hot-toast";
import { Building as BuildingType } from "@/types";

const waterNotificationSchema = z.object({
  building_id: z.string().optional(),
  title: z.string().min(3, "Title must be at least 3 characters"),
  message: z.string().min(10, "Message must be at least 10 characters"),
});

type WaterNotificationFormData = z.infer<typeof waterNotificationSchema>;
...
export default function WaterNotificationsPage() {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<WaterNotificationFormData>({
    resolver: zodResolver(waterNotificationSchema),
  });
  ...
  const onSubmit = async (data: WaterNotificationFormData) => {
    try {
      setIsSending(true);
      
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      toast.success("Water notification sent successfully!");
      reset();
    } catch (error) {
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="px-4 py-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Water Notifications</h1>
              <p className="text-sm text-gray-600">Send water billing and usage alerts</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Monthly Water Bill Available"
                />
                {errors.title && (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent text-gray-900 placeholder-gray-500"
                  placeholder="Enter your water notification message..."
                />
                {errors.message && (
