...
export default function EditApartmentPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const apartmentResponse = await fetch(`/api/apartments/${apartmentId}`);
      ...
      reset({
        ...
        emergency_contact_name: apt.emergency_contact_name || "",
        emergency_contact_phone: apt.emergency_contact_phone || "",
        lease_start_date: apt.lease_start_date || "",
        lease_end_date: apt.lease_end_date || "",
      });

      // Show tenant info section if there's tenant data
      if (apt.tenant_name || apt.tenant_phone || apt.tenant_email) {
  ...
  const onSubmit = async (data: EditApartmentFormData) => {
    try {
      setIsUpdating(true);
      ...
      toast.error(error instanceof Error ? error.message : "Failed to update apartment");
    } finally {
      setIsUpdating(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading apartment...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="px-4 py-4">
          <div className="flex items-center space-x-4">
            <button
              onClick={() => router.push(`/admin/apartments/${apartmentId}`)}
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">
                Edit Unit {apartment.apartment_number}
              </h1>
              <p className="text-sm text-gray-600">Update apartment information</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    Leave blank to mark apartment as vacant
                  </p>
                </div>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <span>{isUpdating ? "Updating..." : "Update Apartment"}</span>
            </button>
